/*
 * N Language JIT Compiler (n-jit.c) - COMPLETE SINGLE FILE v2.0
 * 
 * 철학:
...
ASTNode* parse_primary(Parser* p) {
    ...
    if (p->curr.type == TOK_IDENT) {
        // "Space is the dot" / Function call lookahead
...
void gen_expr(JITBuffer* buf, ASTNode* node) {
    ...
    if (node->type == NODE_INT) {
        ...
        // Find function
        void* func_addr = NULL; int param_count = 0;
        for(int i=0; i<g_func_count; i++) {
            if(strcmp(g_funcs[i].name, node->data.call.name) == 0) {
                func_addr = g_funcs[i].code; param_count = g_funcs[i].param_count; break;
            }
        }
        if (!func_addr) return; // Error handling omitted for brevity
...
void gen_stmt(JITBuffer* buf, ASTNode* node) {
    ...
    if (node->type == NODE_VAR_DECL) {
        ...
        jit_emit(buf, 0x48); jit_emit(buf, 0x89); jit_emit(buf, 0xEC); // mov rsp, rbp
        jit_emit(buf, 0x5D); // pop rbp
        jit_emit(buf, 0xC3); // ret
    } else if (node->type == NODE_FUNC_DEF) {
        // Compile function to a separate buffer
        ...
        // Map parameters to stack offsets (rbp + 16, rbp + 24, etc.)
        ...
        jit_emit(func_buf, 0x48); jit_emit(func_buf, 0x31); jit_emit(func_buf, 0xC0); // xor rax, rax
