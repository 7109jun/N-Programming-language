/*
 * N Language JIT Compiler (n-jit.c) - COMPLETE SINGLE FILE
 * 
 * 철학:
 * - Space is the dot (공백으로 객체/함수 접근)
 * - 인라인 주석 금지 (#은 줄 시작에서만)
 * - 연산자 양옆 공백 필수
 * - .txt 파일 내 <N Language = UTF8> 태그 실행
...
typedef enum {
    TOK_EOF, TOK_LET, TOK_PRINT, TOK_IF, TOK_ELSE, TOK_WHILE,
...
typedef enum { NODE_INT, NODE_STR, NODE_IDENT, NODE_BINOP, NODE_VAR_DECL, NODE_PRINT, NODE_BLOCK, NODE_IF, NODE_WHILE } NodeType;
...
void gen_expr(JITBuffer* buf, ASTNode* node) {
    ...
    if (node->type == NODE_INT) {
        ...
        if (op == '+' || op == '-' || op == '*' || op == '/') {
            ...
            // Comparison: cmp rcx, rax (rcx=left, rax=right)
            jit_emit(buf, 0x48); jit_emit(buf, 0x39); jit_emit(buf, 0xC1);
            if (op == '=') jit_emit(buf, 0x0F); jit_emit(buf, 0x94); jit_emit(buf, 0xC0); // sete al
            else if (op == '<') jit_emit(buf, 0x0F); jit_emit(buf, 0x9C); jit_emit(buf, 0xC0); // setl al
            else if (op == '>') jit_emit(buf, 0x0F); jit_emit(buf, 0x9F); jit_emit(buf, 0xC0); // setg al
            jit_emit(buf, 0x48); jit_emit(buf, 0x0F); jit_emit(buf, 0xB6); jit_emit(buf, 0xC0); // movzx rax, al
        }
    }
}
...
void run_n_code(const char* code) {
    g_output_len = 0; g_output_buffer[0] = '\0'; g_var_count = 0; g_stack_off = 0; g_string_count = 0;
